...
(function () {
  ...
  // Сколько рядов у клавиатуры. Струн на одну больше: они идут
  // по кромкам и по просветам между рядами.
  var РЯДОВ_КНОПОК = 4;

  (function buildStrings() {
    var box = byId('strings');
    // Числа берём из стилей, чтобы они были в одном месте
    var стиль = getComputedStyle(document.documentElement);
    var ШАГ = parseFloat(стиль.getPropertyValue('--panel-step'));
    var КНОПКА = parseFloat(стиль.getPropertyValue('--panel-key'));
    var просвет = ШАГ - КНОПКА;                 // между рядами кнопок
    var блок = РЯДОВ_КНОПОК * КНОПКА + (РЯДОВ_КНОПОК - 1) * просвет;
    var панель = (РЯДОВ_КНОПОК + 1) * ШАГ;
    var верхБлока = (панель - блок) / 2;        // клавиатура стоит по центру

    // Пять струн на четыре ряда кнопок:
    //   первая лежит на верхней кромке верхнего ряда,
    //   последняя — на нижней кромке нижнего,
    //   остальные попадают в просветы между рядами.
    var count = РЯДОВ_КНОПОК + 1;
    for (var i = 0; i < count; i++) {
      var y;
      if (i === 0) y = верхБлока;
      else if (i === count - 1) y = верхБлока + блок;
      else y = верхБлока + i * ШАГ - просвет / 2;
  ...
  (function buildKeys() {
    ...
    var perRow = narrowPanel ? 8 : 16;
    var РЯДОВ = РЯДОВ_КНОПОК;
    for (var r = 0; r < РЯДОВ; r++) {
      var row = el('div', 'keys__row');
      row.style.setProperty('--per', perRow);
      // Верхние ряды уходят влево, нижние вправо, на одинаковую величину:
      // косина остаётся, блок симметричен.
      row.style.transform = 'translateX(' + ((r - (РЯДОВ - 1) / 2) * 4) + 'px)';
      for (var k = 0; k < perRow; k++) {
        // Светлые и тёмные идут столбцами: полосы читаются сверху вниз,
        // вдоль грифа, а не поперёк.
        var key = el('button', 'keys__btn' + (k % 2 ? ' keys__btn--dark' : ''));
        key.type = 'button';
